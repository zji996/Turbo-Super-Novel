...
class NovelProject(Base):
    ...
    status: Mapped[str] = mapped_column(String(32), index=True)  # CREATED, PARSING, READY, PROCESSING, COMPLETED, FAILED
...
class NovelScene(Base):
    ...
    image_prompt: Mapped[str | None] = mapped_column(Text, nullable=True)  # Prompt for image generation
    status: Mapped[str] = mapped_column(String(32), index=True)  # PENDING, TTS_RUNNING, etc.
...
class NovelPipeline(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    project_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), index=True)
    pipeline_type: Mapped[str] = mapped_column(String(32))  # FULL, TTS_ONLY, IMAGEGEN_ONLY, etc.
    status: Mapped[str] = mapped_column(String(32), index=True)  # PENDING, RUNNING, COMPLETED, FAILED, CANCELLED
