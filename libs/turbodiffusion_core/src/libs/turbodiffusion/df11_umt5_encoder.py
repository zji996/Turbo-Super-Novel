...
def _resolve_parent_and_attr(root: torch.nn.Module, dotted: str) -> tuple[torch.nn.Module, str]:
    if "." not in dotted:
        return root, dotted
...
class DF11Config:
    ...
    def from_dir(cls, df11_dir: Path) -> "DF11Config":
        ...
        return cls(
            ...
            pattern_dict={str(k): [str(x) for x in v] for k, v in dict(cfg["pattern_dict"]).items()},
        )
    ...
    def __init__(
        ...
        *,
        ...
        with torch.device("meta"):
            self._model: torch.nn.Module = umt5_xxl(encoder_only=True, dtype=torch.bfloat16, device="meta")
    ...
    def _load_df11_weights(self) -> None:
        ...
        for full_key, tensor in tensors.items():
            ...
            if attr == "encoded_exponent":
                parent.register_forward_pre_hook(get_hook(threads_per_block, bytes_per_thread))
    ...
    def __call__(self, prompts: Union[str, List[str]]) -> torch.Tensor:
        ...
        for emb, length in zip(context, seq_lens):
            if length > self.max_length:
                ...
                zeros = torch.zeros(self.max_length - length, emb.shape[1], device=emb.device, dtype=emb.dtype)
