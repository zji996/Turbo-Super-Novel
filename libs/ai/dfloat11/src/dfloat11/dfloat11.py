...
_ptx_resource = files(__package__).joinpath("decode.ptx")
with as_file(_ptx_resource) as _ptx_path:
    _decode = cp.RawModule(path=str(_ptx_path)).get_function("decode")
...
offloaded_tensor_names = ("encoded_exponent", "sign_mantissa")
    ...
    def decode_hook(module, _):
        ...
        if hasattr(module, "offloaded_tensors"):
            for tensor_name, tensor in module.offloaded_tensors.items():
                if not (
                    hasattr(module, tensor_name)
                    and (getattr(module, tensor_name).device == device)
                ):
                    module.register_buffer(
                        tensor_name, tensor.to(device, non_blocking=True)
                    )
            ...
            if tensor_name in model.state_dict():
                ...
                for i, part in enumerate(parts[:-1]):
                    ...
                    if parts[-1] == "split_positions":
                        ...
                        if (
                            cpu_offload
                            and (cpu_offload_blocks is None or cpu_offload_blocks > 0)
                            and parts[-1] in offloaded_tensor_names
                        ):
                            if not hasattr(module, "offloaded_tensors"):
                                setattr(module, "offloaded_tensors", {})

                            module.offloaded_tensors[parts[-1]] = (
                                tensor_value.pin_memory()
                                if pin_memory
                                else tensor_value
                            )

                            if (
                                (cpu_offload_blocks is not None)
                                and (cpu_offload_blocks > 0)
                                and (
                                    len(module.offloaded_tensors)
                                    == len(offloaded_tensor_names)
                                )
                            ):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        Args:
            ...
            pattern_dict: Dictionary mapping regex patterns to submodule lists
            allow_download: Allow downloading model files from Hugging Face Hub when a local path is missing
        ...
        """
        # Resolve model path. This fork does not auto-download at runtime unless explicitly allowed.
        if from_single_file:
            if os.path.isfile(dfloat11_model_name_or_path):
                ...
                raise IsADirectoryError(
                    f'Expected `dfloat11_model_name_or_path` to be the path to a safetensors file, but found a directory: "{dfloat11_model_name_or_path}".'
                )
            else:
                raise FileNotFoundError(
                    f'The file "{dfloat11_model_name_or_path}" does not exist.'
                )
        else:
            if os.path.exists(dfloat11_model_name_or_path):
                ...
                if not allow_download:
                    raise FileNotFoundError(
                        f'"{dfloat11_model_name_or_path}" does not exist. '
                        "This project does not auto-download model weights at runtime; "
                        "download them via a script and pass a local directory path, or set allow_download=True."
                    )

                dfloat11_model_path = dfloat11_model_name_or_path.replace("/", "__")
                if not os.path.exists(dfloat11_model_path):
                    snapshot_download(
                        dfloat11_model_name_or_path, local_dir=dfloat11_model_path
                    )
        ...
        if bfloat16_model:
            if from_single_file:
                ...
                with open(
                    os.path.join(dfloat11_model_path, "config.json"),
                    "r",
                    encoding="utf-8",
                ) as f:
                    config = json.load(f)
        ...
        if isinstance(config, dict) and "dfloat11_config" in config:
            ...
            raise AttributeError(
                '"dfloat11_config" not found: it is expected to be found in the config file or passed as an argument.'
            )
        ...
        if device:
            ...
            no_split_classes = get_no_split_classes(
                model, dfloat11_config["pattern_dict"]
            )
...
def compress_model(
    ...
    for pattern, attr_names in pattern_dict.items():
        ...
        for full_name, sub_module in model.named_modules():
            if re.fullmatch(pattern, full_name):
                ...
                encoded, other_8bits, output_positions, gaps, split_positions = (
                    encode_weights(
                ...
                if check_correctness:
                    ...
                    (
                        ...
                        map(
                            ...
                            [luts, encoded, other_8bits, output_positions, gaps],
                        )
                    )
                    cuda_outputs = torch.empty(
                        n_elements, dtype=torch.bfloat16, device=device
                    )

                    blocks_per_grid = (
                        int(
                            np.ceil(n_bytes / (threads_per_block[0] * bytes_per_thread))
                        ),
                    )
                    ...
                    _is_correct = (
                        (torch.cat(weights) == cuda_outputs.cpu()).all().item()
                    )
                    if _is_correct:
                        print(
                            "✅ Correctness check passed: decompressed weights match the original weights."
                        )
                    else:
                        raise RuntimeError(
                            f'❌ Correctness check failed: The decompressed weights do not match the original weights for module "{full_name}".'
                        )
                ...
                if not save_single_file:
                    ...
                    state_dict = {
                        f"{full_name}.{key}": value for key, value in state_dict.items()
                    }

                    save_file(
                        state_dict,
                        os.path.join(
                            save_path, full_name.replace(".", "_") + ".safetensors"
                        ),
                    )
    ...
    if save_model:
        ...
        if os.path.exists(os.path.join(save_path, "config.json")):
            with open(
                os.path.join(save_path, "config.json"), "r", encoding="utf-8"
            ) as f:
                config = json.load(f)
