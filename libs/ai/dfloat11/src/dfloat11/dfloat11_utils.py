...
def get_32bit_codec(counter):
    ...
    max_len = 0
    for _, (code_len, _) in table.items():
            ...
            if isinstance(key, int):
                ...
                if bin_val.startswith(p):
                    ...
                    if dict_key in bytes_dict and bytes_dict[dict_key] != dict_value:
                        raise ValueError(
                            f"Key {dict_key} already exists in {bytes_dict}"
                        )
                    else:
...
def encode(data, codec, bytes_per_thread, threads_per_block):
    ...
    for s in tqdm(data):
        if total_size // (8 * bytes_per_thread) + 1 > len(gaps):
            gaps.append(
                total_size
                - total_size // (8 * bytes_per_thread) * (8 * bytes_per_thread)
            )

        if total_size // (8 * bytes_per_thread * threads_per_block) + 1 > len(
            output_positions
        ):
    ...
    blocks_per_grid = int(
        np.ceil(len(encoded) / (threads_per_block * bytes_per_thread))
    )
...
def encode_weights(weights, codec, bytes_per_thread, threads_per_block):
    split_positions = torch.cumsum(
        torch.LongTensor(list(map(lambda x: x.numel(), weights))), dim=0
    )[:-1]
    ...
    encoded, gaps, output_positions = list(
        map(
            torch.from_numpy,
    ...
    print(
        f"Compression factor: {(encoded.numel() + other_8bits.numel() + output_positions.numel() * 4 + gaps.numel()) * 100 / (W_combined.numel() * 2):.2f}%"
    )
