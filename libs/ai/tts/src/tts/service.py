...
def get_tts_provider(provider_name: str = "edge", **kwargs) -> TTSProvider:
    ...
    if name in {"glm_tts", "glm-tts", "glmtts"} and "glm_tts" not in _PROVIDERS:
        try:
            from .providers.glm_tts import GLMTTSProvider

            register_provider("glm_tts", GLMTTSProvider)
        except ImportError as e:
            raise TTSError(
                "GLM-TTS not available. Ensure worker deps are installed (uv sync --project apps/worker). "
                f"Error: {e}"
            ) from e
