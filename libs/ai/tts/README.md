...
- 先用 ModelScope 下载权重到 `<MODELS_DIR>/tts/GLM-TTS`：`uv run --project apps/api --group tools scripts/download_glm_tts_models.py`
- 推理时需要提供 `prompt_wav` + `prompt_text`（零样本克隆）
