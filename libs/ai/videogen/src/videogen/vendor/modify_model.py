...
def _torch_apply_rotary_emb(
    ...
    **_: object,
    ...
    if cos.ndim != 2 or sin.ndim != 2:
        raise ValueError(
            f"Expected cos/sin to have shape [S, D/2], got {tuple(cos.shape)} / {tuple(sin.shape)}"
        )
    if cos.shape != sin.shape:
        raise ValueError(
            f"cos and sin shapes must match, got {tuple(cos.shape)} vs {tuple(sin.shape)}"
        )
    if cos.shape[0] != seq_len:
        raise ValueError(
            f"cos/sin seq_len must match x, got {cos.shape[0]} vs {seq_len}"
        )
    ...
    cos_b = cos.to(device=x.device, dtype=x.dtype).reshape(
        1, seq_len, 1, rotary_dim // 2
    )
    sin_b = sin.to(device=x.device, dtype=x.dtype).reshape(
        1, seq_len, 1, rotary_dim // 2
    )
    ...
    if interleaved:
        ...
        out_rot = torch.stack((out0, out1), dim=-1).reshape(
            batch_size, seq_len, n_heads, rotary_dim
        )
    else:
...
def _replace_attention(
    *, model: torch.nn.Module, attention_type: str, sla_topk: float
) -> torch.nn.Module:
        ...
        if type(module) is WanSelfAttention2pt2:
            ...
            if attention_type == "sla":
                module.attn_op.local_attn = SLA(
                    head_dim=head_dim, topk=sla_topk, BLKQ=128, BLKK=64
                )
                continue
...
def _replace_linear_with_int8(
    *, model: torch.nn.Module, skip_layer: str = "proj_l"
) -> torch.nn.Module:
    """
    ...
    class _SafeInt8Linear(torch.nn.Module):
        def __init__(
            ...
            *,
            ...
            self.register_buffer(
                "scale", torch.empty((row_blocks, col_blocks), dtype=torch.float32)
            )
            if bias:
...
def create_model(dit_path: str, args: argparse.Namespace) -> torch.nn.Module:
    ...
    net = _replace_attention(
        model=net, attention_type=attention_type, sla_topk=sla_topk
    )
