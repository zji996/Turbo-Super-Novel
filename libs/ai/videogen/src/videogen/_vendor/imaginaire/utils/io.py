...
def save_image_or_video_multiview(
    ...
    assert isinstance(save_path, str) or hasattr(save_path, "write"), (
        "save_path must be a string or file-like object"
    )
    ...
    log.info(
        f"Saving video to {save_path} with fps {fps} and result shape {save_obj.shape}"
    )
    easy_io.dump(
        save_obj, save_path, file_format="mp4", format="mp4", fps=fps, **kwargs
    )
...
def save_image_or_video(
    ...
    if tensor.shape[1] == 1:
        save_obj = PILImage.fromarray(
            (
                rearrange((tensor.cpu().float().numpy() * 255), "c 1 h w -> h w c")
                + 0.5
            ).astype(np.uint8),
            mode="RGB",
        )
        ...
        easy_io.dump(
            save_obj, save_path, file_format="jpg", format="JPEG", quality=85, **kwargs
        )
    else:
        save_obj = (
            rearrange((tensor.cpu().float().numpy() * 255), "c t h w -> t h w c") + 0.5
        ).astype(np.uint8)
        if isinstance(save_path, str):
            # Check if path already has an extension
            base, ext = os.path.splitext(save_path)
        ...
        if prompts.get("refined_prompt"):
            if isinstance(prompts["refined_prompt"], str):
                ...
                for chunk_id, refined_prompt in enumerate(prompts["refined_prompt"]):
                    f.write(
                        f"[Refined Prompt for chunk {chunk_id}]\n{refined_prompt}\n"
                    )
            else:
