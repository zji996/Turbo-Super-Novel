...
def _make_graphed_callables(
    ...
    flatten_sample_args = []
    for args, kwargs, kwargs_keys in zip(
        sample_args, sample_kwargs, per_callable_kwargs_keys, strict=False
    ):
    ...
    assert len(warmup_func) == len(sample_args), (
    ...
    graph_id = 0
    for func, args, kwargs, fwd_graph in zip(
        callables, sample_args, sample_kwargs, fwd_graphs, strict=False
    ):
    ...
    def make_graphed_autograd_function(
        ...
        def functionalized(*user_args, **user_kwargs):
            ...
            flatten_user_kwargs, _ = _tree_flatten(
                [user_kwargs[key] for key in kwargs_keys]
            )
            func_args = (
                tuple(flatten_user_args) + tuple(flatten_user_kwargs) + module_params
            )
...
def make_graphed_callables_forward(
    ...
    for module in modules:
        assert isinstance(module, torch.nn.Module), (
            f"Graphing for {type(module)} is not supported."
        )
        ...
        for block in blocks:
            ...
            for arg in real_args:
                if arg.dtype == torch.int64:
                    dummy_arg = torch.randint(
                        arg.min(), arg.max() + 1, arg.shape
                    ).type_as(arg)
                else:
            ...
            for name, kwarg in real_kwargs.items():
                if kwarg.dtype == torch.int64:
                    dummy_kwarg = torch.randint(
                        kwarg.min(), kwarg.max() + 1, kwarg.shape
                    ).type_as(kwarg)
                else:
