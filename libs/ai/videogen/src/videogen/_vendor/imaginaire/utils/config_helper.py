...
def override(config: Config, overrides: list[str] | None = None) -> Config:
    ...
    if overrides:
        if overrides[0] != "--":
            raise ValueError(
                'Hydra config overrides must be separated with a "--" token.'
            )
    ...
    def config_from_dict(ref_instance: Any, kwargs: Any) -> Any:
        ...
        if not is_type:
            ...
            for f in keys:
                resolved_kwargs[f] = config_from_dict(
                    getattr(ref_instance, f), kwargs[f]
                )
            try:
                new_instance = type(ref_instance)(**resolved_kwargs)
            except Exception as e:
                log.error(
                    f"Error when constructing {type(ref_instance)} with {resolved_kwargs}"
                )
                log.error(e)
                raise e
            return new_instance
...
def get_config_module(config_file: str) -> str:
    if not config_file.endswith(".py"):
        ...
        log.error(
            "Please provide the path to the Python config file (relative to the Imaginaire4 root)."
        )
    assert os.path.isfile(config_file), (
        f"Imaginaire4 config file ({config_file}) not found."
    )
...
def import_all_modules_from_package(
    package_path: str, reload: bool = False, skip_underscore: bool = True
) -> None:
    """
    ...
    """
    log.critical(
        f"{'Reloading' if reload else 'Importing'} all modules from package {package_path}"
    )
    ...
    def import_modules_recursively(directory: str, prefix: str) -> None:
        ...
        for _, module_name, is_pkg in pkgutil.iter_modules([directory]):
            if skip_underscore and module_name.startswith("_"):
                log.debug(
                    f"Skipping module {module_name} as it starts with an underscore"
                )
                continue

            full_module_name = f"{prefix}.{module_name}"
            log.debug(
                f"{'Reloading' if reload else 'Importing'} module {full_module_name}"
            )
