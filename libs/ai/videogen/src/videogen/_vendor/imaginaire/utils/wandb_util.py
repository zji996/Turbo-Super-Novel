...
def init_wandb(config: Config, model: ImaginaireModel) -> None:
    ...
    local_safe_yaml_fp = LazyConfig.save_yaml(
        config, os.path.join(config_job.path_local, "config.yaml")
    )
...
def _read_wandb_id(
    config_job: JobConfig, config_checkpoint: CheckpointConfig
) -> str | None:
    ...
    if config_checkpoint.load_from_object_store.enabled:
        object_store_loader = object_store.ObjectStore(
...
def _write_wandb_id(
    config_job: JobConfig, config_checkpoint: CheckpointConfig, wandb_id: str
) -> None:
    ...
    if config_checkpoint.save_to_object_store.enabled:
        object_store_saver = object_store.ObjectStore(
