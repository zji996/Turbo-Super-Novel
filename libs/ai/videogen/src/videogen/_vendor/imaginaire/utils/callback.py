...
class CallBackGroup:
    ...
    def __init__(self, config: Config, trainer: ImaginaireTrainer) -> None:
        ...
        if callback_configs:
            if isinstance(callback_configs, list) or isinstance(
                callback_configs, omegaconf.listconfig.ListConfig
            ):
                warnings.warn(
                ...
                callback_configs = {
                    f"callback_{i}": v for i, v in enumerate(callback_configs)
                }
            for callback_name, current_callback_cfg in callback_configs.items():
                if "_target_" not in current_callback_cfg:
                    ...
                    continue
                log.critical(
                    f"Instantiating callback {callback_name}: {current_callback_cfg}"
                )
...
class Callback:
    ...
    def __init__(
        self, config: Config | None = None, trainer: ImaginaireTrainer | None = None
    ):
    ...
    def on_train_start(self, model: ImaginaireModel, iteration: int = 0) -> None:
        ...
        if model.config.ema.enabled:
            assert hasattr(model, "ema"), (
                "EMA should be initialized from ImaginaireModel"
            )
