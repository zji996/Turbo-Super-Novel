...
def maybe_enable_profiling(config, *, global_step: int = 0):
    ...
    if enable_profiling:
        ...
        def trace_handler(prof):
            ...
            if (
                config.trainer.profiling.first_n_rank < 0
                or rank < config.trainer.profiling.first_n_rank
            ):
                prof.export_chrome_trace(
                    f"{curr_trace_dir}/rank{rank}_trace.json.gz"
                )  # saved as gz to save space
            log.info(
