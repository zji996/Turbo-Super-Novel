...
class FastEmaModelUpdater:
    ...
    def copy_to(self, src_model: torch.nn.Module, tgt_model: torch.nn.Module) -> None:
        for tgt_params, src_params in zip(
            tgt_model.parameters(), src_model.parameters(), strict=False
        ):
...
class EMAModelTracker(torch.nn.Module):
    ...
    @torch.no_grad()
    def update_average(
        self, model: ImaginaireModel, iteration: int | None = None
    ) -> None:
        del iteration
        target_list = []
        source_list = []
    ...
    def initialize_multi_rank_ema(
        ...
        if USE_MEGATRON and parallel_state.is_initialized():
            cur_dp_rank = parallel_state.get_data_parallel_rank(
            ...
            log.critical(
                f"using torch.distributed for EMA initialization. DP RANK: {cur_dp_rank}",
                rank0_only=False,
            )
