...
def _pretty_print_attrs_instance(
    obj: object, indent: int = 0, use_color: bool = False
) -> str:
    """
...
def pretty_print_overrides(
    overrides: list[str] | None = None, use_color: bool = False
) -> str:
    """
    ...
    for override in overrides:
        ...
        if use_color:
            lines.append(
                "   "
                + Color.cyan("* ")
                + Color.green(attribute_name)
                + ": "
                + Color.yellow(attribute_value)
            )
        else:
...
@attrs.define(
    slots=False
)  # slots=False is required for make_freezable. See the make_freezable notes for more info.
...
class Profiling:
    ...
    profile_freq: int = 1
    first_n_rank: int = (
        8  # -1 means all ranks, n means first n ranks dumpy profiling info
    )
