...
) -> OrderedDict[str, Any] | list[Any] | Any:
...
class LazyCall(Generic[T]):
    ...
    def __call__(self, **kwargs) -> LazyDict[T]:
        ...
        return cast(
            LazyDict[T],
            DictConfig(content=_final_params, flags={"allow_objects": True}),
        )
...
class LazyConfig:
    ...
    def save_pkl(cfg, filename: str) -> str:
        ...
        try:
            ...
            log.error(
                f"Failed to save config to {filename}: {e}. Trying dill or cloudpickle instead"
            )
            if dill_pickle:
                try:
                    with PathManager.open(filename, "wb") as f:
                    ...
                    if cloudpickle:
                        try:
                            ...
                            log.warning(
                                f"Config is saved using cloudpickle at {filename}."
                            )
                        except Exception as e:
                            log.error(f"Failed to save config to {filename}: {e}.")
                    else:
                        log.error(
                            "cloudpickle is not available. Cannot save the config."
                        )
                        raise e
    ...
    def save_yaml(cfg, filename: str) -> str:
        ...
        def serialize_config(config):
            if isinstance(config, DictConfig):
                ...
                raise NotImplementedError(
                    "Input config must be a DictConfig or ListConfig."
                )
            return config
        ...
        config_omegaconf = DictConfig(
            content=config_dict, flags={"allow_objects": True}
        )
