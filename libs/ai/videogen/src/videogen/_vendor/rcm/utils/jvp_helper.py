...
def torch_attention_op_withT(
    q_B_S_H_D_withT: TensorWithT,
    k_B_S_H_D_withT: TensorWithT,
    v_B_S_H_D_withT: TensorWithT,
):
...
class _SeqAllToAllQKVWithT(torch.autograd.Function):
    ...
    def backward(
        ...
        q_grad, t_q_grad, k_grad, t_k_grad, v_grad, t_v_grad = (
...
class _SeqAllToAllOutputWithT(torch.autograd.Function):
    ...
    def forward(
        ...
        ctx.cp_stream = cp_stream
        ctx.local_seq_2_local_head = local_seq_2_local_head
        output, t_output = async_a2a_communicate(
            [output, t_output], cp_size, group, cp_stream, local_seq_2_local_head
        )
    ...
    def backward(
        ctx: Any, *grad_output: Tensor
    ) -> Tuple[None, Tensor, Tensor, None, None, None]:
...
class MinimalA2AAttnOpWithT(torch.nn.Module):
    def __init__(
        self, local_attn_T: Union[nn.Module, Callable] = torch_attention_op_withT
    ):
        super().__init__()
    ...
    def forward(
        self,
        query_withT: TensorWithT,
        key_withT: TensorWithT,
        value_withT: TensorWithT,
        *args: Any,
        **kwargs,
    ) -> TensorWithT:
        del args, kwargs
        if self.pg is None:
            output_B_S_H_D, t_output_B_S_H_D = self.local_attn_T(
                query_withT, key_withT, value_withT
            )
        else:
...
def naive_attention_op(q_B_S_H_D, k_B_S_H_D, v_B_S_H_D):
    ...
    return rearrange(
        naive_scaled_dot_product_attention(q_B_H_S_D, k_B_H_S_D, v_B_H_S_D),
        "b h ... l -> b ... h l",
    )
