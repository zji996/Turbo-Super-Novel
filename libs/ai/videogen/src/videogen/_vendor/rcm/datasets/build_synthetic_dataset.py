...
def main(args):
    ...
    neg_text_emb = (
        get_umt5_embedding(
            checkpoint_path=args.text_encoder_path, prompts=args.negative_prompt
        )
    ...
    for shard_id in my_shards:
        ...
        if os.path.exists(tmp_path):
            try:
                ...
                if processed_indices:
                    print(
                        f"[Rank {rank}] Found {len(processed_indices)} completed items in tmp file for shard {shard_id}."
                    )

            except (tarfile.ReadError, EOFError) as e:
                print(
                    f"[Rank {rank}] Tmp file for shard {shard_id} is corrupted ({e}). Deleting and starting over."
                )
                os.remove(tmp_path)
        ...
        for i in tqdm(range(start, end, args.batch_size), disable=(rank != 0)):
            ...
            indices_to_process = [
                idx for idx in original_indices_batch if idx not in processed_indices
            ]
            ...
            text_embs_batch = (
                get_umt5_embedding(
                    checkpoint_path=args.text_encoder_path, prompts=prompts_to_process
                )
                .to(dtype=torch.bfloat16)
                .cuda()
            )

            latents_batch = sample_latents_from_prompts(
                indices_to_process, text_embs_batch, neg_text_emb, net, tokenizer, args
            )
            ...
            try:
                with tarfile.open(tmp_path, "a") as tar:
                    for j, original_idx in enumerate(indices_to_process):
                        ...
                        write_to_tar(
                            tar, f"{key_prefix}.latent.pt", latent_buffer.getvalue()
                        )

                        embed_buffer = io.BytesIO()
                        torch.save(text_emb.cpu(), embed_buffer)
                        write_to_tar(
                            tar, f"{key_prefix}.embed.pt", embed_buffer.getvalue()
                        )

                        write_to_tar(
                            tar, f"{key_prefix}.prompt.txt", prompt.encode("utf-8")
                        )
...
def sample_latents_from_prompts(indices, text_embs, neg_text_emb, net, tokenizer, args):
    ...
    uncondition = {
        "crossattn_emb": repeat(
            neg_text_emb.to(**tensor_kwargs), "b l d -> (k b) l d", k=batch_size
        )
    }
    ...
    unshifted_sigma_max = sigma_max / (
        args.timestep_shift - (args.timestep_shift - 1) * sigma_max
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Batch size for sampling. For best efficiency, choose a divisor of samples_per_shard.",
    )
    ...
    parser.add_argument(
        "--num_steps", type=int, default=100, help="Number of sampling steps"
    )
    parser.add_argument(
        "--sigma_max",
        type=int,
        default=5000,
        help="Initial timestep represented by EDM sigma",
    )
    parser.add_argument(
        "--sampler", choices=["Euler", "UniPC"], default="Euler", help="Sampler"
    )
    parser.add_argument("--guidance_scale", type=float, default=5.0, help="CFG scale")
    parser.add_argument(
        "--timestep_shift", type=float, default=3.0, help="Timestep shift as in Wan"
    )
    parser.add_argument(
        "--dit_path",
        type=str,
        default="assets/checkpoints/Wan2.1-T2V-14B.pth",
        help="Path to the video diffusion model.",
    )
    parser.add_argument(
        "--vae_path",
        type=str,
        default="assets/checkpoints/Wan2.1_VAE.pth",
        help="Path to the Wan2.1 VAE.",
    )
    parser.add_argument(
        "--text_encoder_path",
        type=str,
        default="assets/checkpoints/models_t5_umt5-xxl-enc-bf16.pth",
        help="Path to the umT5 text encoder.",
    )
    parser.add_argument(
        "--num_frames", type=int, default=81, help="Number of frames to generate"
    )
    parser.add_argument(
        "--negative_prompt",
        type=str,
