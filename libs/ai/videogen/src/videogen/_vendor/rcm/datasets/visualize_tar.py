...
def main(args):
    ...
    with tarfile.open(args.tar_path, "r") as tar:
        for member_name in tqdm(latent_files, desc="Decoding"):
            ...
            latent_tensor = torch.load(
                io.BytesIO(member_file.read()), map_location=device
            )
    ...
    if num_videos != grid_rows * grid_cols:
        print(
            f"Warning: The number of videos ({num_videos}) doesn't fit a perfect {grid_rows}x{grid_cols} grid. Rearranging may be imperfect."
        )

    to_show = rearrange(
        to_show, "(rows cols) b c t h w -> c t (rows h) (cols b w)", cols=grid_cols
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Decode latent samples from a .tar file into a video grid."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the input .tar file containing latent samples.",
    )
    parser.add_argument(
        "--save_path",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Path to the Wan2.1 VAE model weights.",
    )
    parser.add_argument(
        "--num_samples",
        type=int,
        default=16,
        help="Number of samples to decode from the tar file. Set to 0 to decode all.",
    )
    parser.add_argument(
        "--grid_cols",
        type=int,
        default=4,
        help="Number of columns in the output video grid.",
    )
    parser.add_argument(
