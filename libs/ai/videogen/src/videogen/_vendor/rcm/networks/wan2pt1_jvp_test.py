...
def test_equivalent_jvp_naive_vs_flash():
    dtype = torch.float16
    net_jvp = (
        instantiate(mini_net_jvp, sac_config=SACConfig(mode=CheckpointMode.NONE))
        .cuda()
        .to(dtype=dtype)
    )
    net_jvp.eval()
    net_jvp_naive = (
        instantiate(mini_net_jvp_naive, sac_config=SACConfig(mode=CheckpointMode.NONE))
        .cuda()
        .to(dtype=dtype)
    )
    net_jvp_naive.eval()
    ...
    fn_naive = functools.partial(
        net_jvp_naive.forward,
        crossattn_emb=crossattn_emb_B_T_D,
        fps=fps_B,
        padding_mask=padding_mask_B_T_H_W,
    )

    output_BT_naive, t_output_BT_naive = torch.func.jvp(
        fn_naive, (x_B_C_T_H_W, noise_labels_BT), (t_x_B_C_T_H_W, t_noise_labels_BT)
    )

    torch.testing.assert_close(
        output_BT_withoutT, output_BT_naive, rtol=1e-3, atol=1e-3
    )
