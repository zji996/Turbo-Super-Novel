...
def rope_apply(x, video_size: VideoSize, freqs, fused):
    ...
    if fused:
        ...
        rotated = apply_rotary_emb_torch(
            x.to(torch.float32), cos, sin, interleaved=True
        )
...
class WanSelfAttention(JVP):
    def __init__(self, dim, num_heads, qk_norm=True, eps=1e-6, naive_attn=False):
        ...
        self.attn_op = (
            MinimalA2AAttnOp(local_attn=naive_attention_op)
            if naive_attn
            else MinimalA2AAttnOp(local_attn=torch_attention_op)
        )
    ...
    def _forward(self, x: torch.Tensor, seq_lens, video_size: VideoSize, freqs):
        ...
        x = self.attn_op(
            rope_apply(q, video_size, freqs, fused=False),
            rope_apply(k, video_size, freqs, fused=False),
            v,
        )
...
class WanI2VCrossAttention(WanSelfAttention):
    ...
    def _forward_jvp(self, x: TensorWithT, context, context_lens):
        ...
        (q, k, v, k_img, v_img), (t_q, t_k, t_v, t_k_img, t_v_img) = torch.func.jvp(
            qkv_fn, (x,), (t_x,)
        )
...
class WanAttentionBlock(JVP):
    def __init__(
        ...
        self.cross_attn = WAN_CROSSATTENTION_CLASSES[cross_attn_type](
            dim, num_heads, qk_norm, eps, naive_attn
        )
    ...
    def _forward(
        self, x, e, seq_lens, video_size: VideoSize, freqs, context, context_lens
    ):
        r"""
        Args:
            x(Tensor): Shape [B, L, C]
    ...
    def _forward_jvp(
        ...
        with amp.autocast("cuda", dtype=torch.float32):
            x, t_x = torch.func.jvp(
                post_cross_attn_fn,
                (x, z, e[3], e[4], e[5]),
                (t_x, t_z, t_e[3], t_e[4], t_e[5]),
            )
        return (x, t_x.detach())
...
class WanModel_JVP(JVP):
    ...
    def _forward(
        ...
        **kwargs,
        ...
        if self.model_type == "i2v" or self.model_type == "flf2v":
            assert (
                frame_cond_crossattn_emb_B_L_D is not None and y_B_C_T_H_W is not None
            )
        ...
        video_size = VideoSize(
            T=x_B_T_H_W_D.shape[1], H=x_B_T_H_W_D.shape[2], W=x_B_T_H_W_D.shape[3]
        )
        ...
        # time embeddings
        with amp.autocast("cuda", dtype=torch.float32):
            e_B_D = self.time_embedding(
                sinusoidal_embedding_1d(self.freq_dim, t_B).float()
            )
        ...
        if frame_cond_crossattn_emb_B_L_D is not None:
            context_clip = self.img_emb(
                frame_cond_crossattn_emb_B_L_D
            )  # bs x 257 (x2) x dim
        ...
        **kwargs,
        ...
        if y_B_C_T_H_W is not None:
            ...
            t_x_B_C_T_H_W = torch.cat(
                [t_x_B_C_T_H_W, torch.zeros_like(y_B_C_T_H_W)], dim=1
            )
        ...
        x_B_T_H_W_D, t_x_B_T_H_W_D = torch.func.jvp(
            self.patch_embedding, (x_B_T_H_W_D,), (t_x_B_T_H_W_D,)
        )
        ...
        # time embeddings
        with amp.autocast("cuda", dtype=torch.float32):
            (e_B_D, e0_B_6_D), (t_e_B_D, t_e0_B_6_D) = torch.func.jvp(
                time_embed_fn, (t_B,), (t_t_B,)
            )
    ...
    def fully_shard(self, mesh, mp_policy):
        ...
        fully_shard(
            self.head, mesh=mesh, mp_policy=mp_policy, reshard_after_forward=False
        )
        fully_shard(
            self.text_embedding,
            mesh=mesh,
    ...
    def enable_context_parallel(self, process_group: Optional[ProcessGroup] = None):
        # pos_embedder
        self.rope_position_embedding.enable_context_parallel(
    ...
    def enable_selective_checkpoint(self, sac_config: SACConfig):
        ...
        log.info(
            f"Enable selective checkpoint with mm_only, for every {sac_config.every_n_blocks} blocks. Total blocks: {len(self.blocks)}"
        )
        ...
        for block_id, block in self.blocks.named_children():
            if int(block_id) % sac_config.every_n_blocks == 0:
                block = ptd_checkpoint_wrapper(
                    block, context_fn=_context_fn, preserve_rng_state=False
                )
                self.blocks.register_module(block_id, block)
        self.register_module(
            "head",
            ptd_checkpoint_wrapper(
                self.head, context_fn=_context_fn, preserve_rng_state=False
            ),
        )
