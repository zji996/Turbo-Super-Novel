...
class DistributedCheckpointer_Distill(DistributedCheckpointer):
            ...
            for key in resume_keys:
                ...
                if key == "model":
                    ...
                    for k, v in optimizer_dict.items():
                        storage_reader = self.get_storage_reader(
                            f"{cur_key_ckpt_full_path}_{k}"
                        )
        ...
        for k, (v, full_checkpoint_path) in to_save_dict.items():
            if k in ["optim", "scheduler"]:
                for key_net, state_dict in v.items():
                    storage_writer = self.get_storage_writer(
                        f"{full_checkpoint_path}_{key_net}"
                    )
                    dcp.save(
                        state_dict,
    ...
    def save(
        ...
        to_save_dict = {
            ...
            "optim": {
                k: OptimizerWrapper(model_dict[k], v).state_dict()
                for k, v in optimizer_dict.items()
            },
