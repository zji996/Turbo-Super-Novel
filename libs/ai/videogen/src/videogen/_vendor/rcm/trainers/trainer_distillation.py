...
class ImaginaireTrainer_Distill:
    ...
    def __init__(self, config):
        ...
        with misc.timer("init_distributed"):
            ...
            if USE_MEGATRON:
                ...
                parallel_state.sequence_parallel = (
                    config.model_parallel.sequence_parallel
                )
    ...
    def train(
        ...
        iteration = self.checkpointer.load(
            model, model.optimizer_dict, model.scheduler_dict, grad_scaler
        )
        grad_accum_iter = 0
        log.critical(
        ...
        if self.config.trainer.distributed_parallelism == "ddp":
            ...
            model_ddp = distributed.parallel_model_wrapper(
                self.config.trainer.ddp, model
            )
    ...
    def training_step(
        ...
        with distributed.ddp_sync_grad(
            model_ddp, grad_accum_iter == self.config.trainer.grad_accum_iter - 1
        ):
            ...
            with self.training_timer("backward"):
                loss_scaled = grad_scaler.scale(
                    loss / self.config.trainer.grad_accum_iter
                )
