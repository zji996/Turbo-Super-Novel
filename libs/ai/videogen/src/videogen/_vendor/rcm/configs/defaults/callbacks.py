...
BASIC_CALLBACKS = dict(
    ...
    low_prec=L(LowPrecisionCallback)(
        config=PLACEHOLDER, trainer=PLACEHOLDER, update_iter=1
    ),
...
def register_callbacks():
    ...
    cs.store(
        group="callbacks",
        package="trainer.callbacks",
        name="dataloading_speed",
        node=SPEED_CALLBACKS,
    )
    cs.store(
        group="callbacks",
        package="trainer.callbacks",
    ...
    cs.store(
        group="callbacks",
        package="trainer.callbacks",
        name="viz_online_sampling_distill",
        node=VIZ_ONLINE_SAMPLING_DISTILL_CALLBACKS,
    )
    cs.store(
        group="callbacks",
        package="trainer.callbacks",
        name="viz_online_sampling_sla",
        node=VIZ_ONLINE_SAMPLING_SLA_CALLBACKS,
    )
