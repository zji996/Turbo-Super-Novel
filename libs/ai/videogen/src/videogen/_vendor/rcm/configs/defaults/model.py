...
FSDP_CONFIG_T2V_DISTILL_RCM = dict(
    ...
    model=L(T2VDistillModel_rCM)(
        config=T2VDistillConfig_rCM(fsdp_shard_size=8), _recursive_=False
    ),
)
...
def register_model():
    ...
    cs.store(
        group="model",
        package="_global_",
        name="fsdp_t2v_distill_rcm",
        node=FSDP_CONFIG_T2V_DISTILL_RCM,
    )
    cs.store(
        group="model", package="_global_", name="fsdp_t2v_sla", node=FSDP_CONFIG_T2V_SLA
    )
