...
def broadcast_condition(
    condition: BaseCondition, process_group: Optional[ProcessGroup] = None
) -> BaseCondition:
    """
...
def concat_condition(cond1, cond2):
    kwargs1, kwargs2 = (
        cond1.to_dict(skip_underscore=False),
        cond2.to_dict(skip_underscore=False),
    )
    kwargs = {}
...
class TextAttr(AbstractEmbModel):
    ...
    def random_dropout_input(
        ...
        keep_mask = torch.bernoulli(
            (1.0 - dropout_rate) * torch.ones(B, device=in_tensor.device)
        ).type_as(in_tensor)
    ...
    def _forward(
        ...
        for emb_name, embedder in self.embedders.items():
            if isinstance(embedder.input_key, str):
                ...
                embedder.input_key, (list, omegaconf.listconfig.ListConfig)
            ):
                emb_out = embedder(
                    *[
                        embedder.random_dropout_input(
