...
def sample_batch_video(
    resolution: str = "512", batch_size: int = 1, num_frames: int = 17
):
    ...
    data_batch = {
        ...
        "videos": torch.randint(
            0, 256, (batch_size, 3, num_frames, h, w), dtype=torch.uint8
        ).cuda(),
        ...
        if self.sample_fix:
            ...
            for prompt in video_prompts:
                ...
                self.kv_prompt_to_emb[prompt] = (
                    get_umt5_embedding(
                        checkpoint_path=model.config.text_encoder_path, prompts=prompt
                    )
    ...
    def sample_fixed(self, trainer, model, data_batch, output_batch, loss, iteration):
        ...
        data_batch = get_sample_batch(
            num_frames=(
                1
                if self.is_image
                else model.tokenizer.get_pixel_num_frames(
                    model.get_num_video_latent_frames()
                )
            ),
        ...
        for prompt, text_emb in self.kv_prompt_to_emb.items():
            ...
            data_batch["t5_text_embeddings"] = repeat(
                text_emb.to(**model.tensor_kwargs),
                "b l d -> (k b) l d",
                k=self.num_samples,
            )
