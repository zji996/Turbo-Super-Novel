...
def validate_scenes(scenes: list[SceneData]) -> list[str]:
    ...
    for scene in scenes:
        if len(scene.text) < 10:
            warnings.append(f"Scene {scene.sequence}: Text too short ({len(scene.text)} chars)")
        
        if len(scene.text) > 5000:
            warnings.append(f"Scene {scene.sequence}: Text very long ({len(scene.text)} chars), may need splitting")
