...
def _read_prompt(prompts_path: Path, index: int) -> str:
    lines = [line.strip() for line in prompts_path.read_text(encoding="utf-8").splitlines() if line.strip()]
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument("--input-index", type=int, default=0, help="Which i2v_input_*.jpg to use (default: 0)")
    parser.add_argument("--prompt-index", type=int, default=0, help="Which line in prompts.txt to use (default: 0)")
    parser.add_argument(
        "--inputs-dir",
        type=Path,
    ...
    out = (data_dir() / "turbodiffusion" / "smoke" / f"i2v_{args.input_index}_{args.prompt_index}.mp4").resolve()
    run_wan22_i2v(
